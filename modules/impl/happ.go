...
func New() (*Engine, error) {
	...
	versionMap := []string{VersionCrypt1, VersionCrypt2, VersionCrypt3, VersionCrypt4}

	for idx, b64RawKey := range PKCS1_KEYS_B64 {
		...
		if err != nil {
			return nil, fmt.Errorf("failed base64 initialization decoding for index %d: %w", idx, err)
		}
		...
		if err != nil {
			...
			if err != nil {
				return nil, fmt.Errorf("failed initialization parse for version %s: %w", versionMap[idx], err)
			}
...
func (p *Engine) Decrypt(link string) (Result, error) {
	...
	for _, keyVersion := range keysToTry {
		...
		decrypted, err := decryptCrypt1to4(encryptedData, priv)
		if err == nil {
			return Result{
				Version:       version,
				UsedKey:       keyVersion,
...
func decryptCrypt1to4(encryptedB64 string, privateKey *rsa.PrivateKey) (string, error) {
	...
	if len(cipherBytes)%keySize != 0 {
		return "", errors.New("cipher text data boundaries are not aligned to block configurations")
	}
